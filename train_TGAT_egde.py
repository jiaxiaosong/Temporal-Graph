...
def run_model(start_timestamp, end_timestamp, state):
    ...
    while start_timestamp + time_index * tbatch_timespan <= end_timestamp:
        ...
        now_local_dst_node = set(np.vectorize(node_index_global2local.get)(now_dst_node))
        ...
        with torch.set_grad_enabled(is_train):
            ...
            now_graph.clear()
            with torch.no_grad():
                src_node, pos_dst_node = new_edge.local_u.to_numpy(), new_edge.local_i.to_numpy()
                ##dim: num_of_new_edge
                neg_dst_node = np.array(np.array([random.sample(now_local_dst_node-set([pos_dst_node[i]]), neg_sampling_ratio) for i in range(len(pos_dst_node))]))
                sample_src_node = np.repeat(src_node, neg_sampling_ratio+1, axis=-1).ravel()
                sample_dst_node = np.concatenate([np.expand_dims(pos_dst_node, axis=-1), neg_dst_node], axis=-1).ravel()
                label = torch.Tensor(([1]+[0]*neg_sampling_ratio)*src_node.shape[0]).to(device)
        ...
        if is_train & (time_index % print_freq == 0):
            print('Train Epoch: [{0}][{1}/{2}][{all_ap.count}({3})], AP {all_ap.val:.4f}({all_ap.avg:.4f}), Acc {all_acc.val:.4f}({all_acc.avg:.4f}), Loss {all_m_loss.val:.2e}({all_m_loss.avg:.2e}), Auc {all_auc.val:.4f}({all_auc.avg:.4f}), F1 {all_f1.val:.4f}({all_f1.avg:.4f})'.format(epoch, time_index, tmp_tbatch_num, new_edge.shape[0], all_ap=all_ap, all_acc=all_acc, all_m_loss=all_m_loss, all_auc=all_auc, all_f1=all_f1))
