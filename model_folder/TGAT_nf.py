...
class TGATLayer_nf(nn.Module):
    ...
    def message_func(self, edges):
        ...
        return {"k":k, "v":v, "src_fea":edges.src['node_h']}
    ...
    def reduce_func(self, nodes):
        ...
        ##self-loop term
        if self.edge_dim:
            self_z = torch.cat([nodes.mailbox['src_fea'][:,-1,:], torch.zeros(node_batch, self.edge_dim, device=self.device), torch.ones(node_batch, self.d_T, device=self.device)*torch.cos(self.phase)], dim=-1).unsqueeze(1)#node_batch, 1, d_model
        else:
            self_z = torch.cat([nodes.mailbox['src_fea'][:,-1,:], torch.ones(node_batch, self.d_T, device=self.device)*torch.cos(self.phase)], dim=-1).unsqueeze(1) #node_batch, 1, d_model
        ...
        k = nodes.mailbox['k'][:,:-1,:].view(node_batch, neightbor_num, self.n_head, -1)#node_batch, neightbor_num, n_head, d_model
        v = nodes.mailbox['v'][:,:-1,:].view(node_batch, neightbor_num, self.n_head, -1)#node_batch, neightbor_num, n_head, d_model
        ...
        ###Merger
        output = self.merger(output, nodes.mailbox['src_fea'][:,-1,:]) ## node_batch, node_dim
