...
class TGATLayer_v2(nn.Module):
    def __init__(self, n_head, node_dim, d_k, d_v, edge_dim=None, dropout=0.1, act=torch.nn.functional.gelu, device="cpu"):
...
class TGAT_nf(nn.Module):
    def __init__(self, num_layers, n_head, node_dim, d_k, d_v, d_T, edge_dim = None, dropout=0.1, act=torch.nn.functional.gelu, device="cpu"):
        super(TGAT_nf, self).__init__()
        self.gnn_layers =  torch.nn.ModuleList([TGATLayer_v2(n_head, node_dim, d_k, d_v, edge_dim, dropout, act, device=device) for _ in range(num_layers)])
